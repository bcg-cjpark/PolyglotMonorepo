...
- (자동) 해당 기능의 코드 경로:
  - 백엔드: `apps/api/**/<feature>/**`, `apps/api/**/domain/<feature>/**`
  - 프론트: `apps/web/src/pages/<Feature>*Page.tsx`, `apps/web/src/services/<feature>.ts`
...
**도메인 엔티티 매칭**:
- `apps/api/domain/src/main/kotlin/**/<feature>/<Entity>.kt` 존재 확인
...
**화면별 Route 매칭**:
- `apps/web/src/App.tsx` 의 `<Route path=...>` 수집
...
**서비스 호출 매칭**:
- `apps/web/src/services/<feature>.ts` 가 PRD API 엔드포인트와 일대일 호출 함수 있는지
- fetch/axios 경로 grep
