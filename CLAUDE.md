...
2. **없다면 `ui-composer` 에이전트 호출** — `.claude/agents/ui-composer.md` 가 Button 패턴을 레퍼런스로 일관된 구조로 새 컴포넌트를 `libs/ui` 에 추가. 그 후 ui-verifier로 자동 smoke test.
...
| React 컴포넌트/페이지/라우트/서비스 (`apps/example-web/src/`, `libs/ui/src/`) | `build` + **`ui-verifier` 에이전트 호출 (Playwright)** |
| CSS/토큰/Tailwind 설정 | dev 서버에서 transform 확인 **+** UX 변화 있으면 `ui-verifier` 호출 |

### UX 변경 시 Task 도구로 `ui-verifier` 호출 필수

React 쪽 변경 후 **브라우저에서 실제로 동작하는지**는 HTTP/빌드 검증만으로는 알 수 없음.
`ui-verifier` 에이전트(`.claude/agents/ui-verifier.md`)가 Playwright로 실제 키보드/마우스
이벤트를 시뮬레이션해서 검증.

**호출 예시:**
> React UserFormPage 수정 끝. Task 도구로 ui-verifier 에이전트 호출해서
> "Name/Email 입력 타이핑, Create 버튼 제출, 생성된 유저가 리스트에 나타나는지" 검증.

**`ui-verifier` 를 건너뛰어도 되는 경우:**
- 백엔드만 수정 (Kotlin, SQL)
- 문서/주석만 변경
...
- 이전 위반 사례: (1) build만 보고 dev 에러 방치, (2) HTTP 200만 보고 "버튼 클릭/입력 동작" 미검증.
