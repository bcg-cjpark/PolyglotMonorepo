...
export const InputStepper = memo(function InputStepper({
  ...
  const inputValue = formatNumber(localValue);
  ...
  return (
    <div className="flex w-full flex-col">
      <div className="flex h-[34px] w-full items-center">
        ...
        {variant === 'range' && (
          <div
            className={`bg-input-surface flex select-none items-center justify-center rounded-l-[6px] border border-r-0 py-2 pl-3 text-[14px] font-normal leading-[18px] ${borderClass} ${disabled ? 'cursor-not-allowed opacity-50' : ''}`}
          >
            ~
          </div>
        )}
        ...
        <input
          ...
          inputMode="decimal"
          className={[
            'bg-input-surface min-w-0 flex-1 border px-3 py-2 text-[14px] font-normal leading-[18px] focus:outline-none focus:ring-0',
            borderClass,
        ...
        {variant === 'unit' && (
          <div
            className={`bg-input-surface flex select-none items-center justify-center border border-l-0 border-r-0 px-3 py-2 text-[14px] font-normal leading-[18px] ${borderClass}`}
          >
            {unitLabel}
          </div>
        )}
        ...
        {/* 증가 버튼 */}
        <div
          className={`bg-input-surface flex cursor-pointer items-center justify-center border px-1 py-[9px] transition-colors hover:bg-gray-50 ${borderClass} ${!canIncrease ? 'cursor-not-allowed opacity-50' : ''}`}
        ...
        {/* 감소 버튼 */}
        <div
          className={`bg-input-surface flex cursor-pointer items-center justify-center rounded-r-[6px] border border-l-0 px-1 py-[9px] transition-colors hover:bg-gray-50 ${borderClass} ${!canDecrease ? 'cursor-not-allowed opacity-50' : ''}`}
      ...
      {/* 에러 메시지 */}
      {error && errorMessage && (
        <div className="text-input-border-error mt-1 text-[12px] font-medium">{errorMessage}</div>
      )}
    </div>
  );
});
